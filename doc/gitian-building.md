...
*Setup instructions for a Gitian build of Aghilbit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aghilbit
...
- [Building Aghilbit](#building-aghilbit)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-aghilbit-host/README.md).
...
Clone the git repositories for aghilbit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aghilbit/aghilbit
```
...
in the aghilbit repository under 'Fetch and build inputs' to install sources which require
...
To build Aghilbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the aghilbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aghilbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/aghilbit/aghilbit
...
For example:
```bash
URL=https://github.com/laanwj/aghilbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aghilbit=${COMMIT} --url aghilbit=${URL} ../aghilbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aghilbit=${COMMIT} --url aghilbit=${URL} ../aghilbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aghilbit=${COMMIT} --url aghilbit=${URL} ../aghilbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the aghilbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aghilbit/contrib/gitian-descriptors/*|sort|uniq )
...
MBTPATH=/some/root/path/aghilbit.git
SIGPATH=/some/root/path/aghilbit-detached-sigs.git

./bin/gbuild --url aghilbit=${MBTPATH},signature=${SIGPATH} ../aghilbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aghilbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aghilbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aghilbit-osx-build.assert
```
...
[aghilbit/gitian.sigs](https://github.com/aghilbit/gitian.sigs/) repository, or if that's not possible create a pull
