...
###   This script attempts to download the signature file SHA256SUMS.asc from aghilbit.org
...
WORKINGDIR="/tmp/aghilbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://aghilbit.org/bin/"
VERSIONPREFIX="aghilbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'aghilbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Aghilbit Core binary release signing key installed?"
   fi
